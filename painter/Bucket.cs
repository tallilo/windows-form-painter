...
{
    [Serializable]
    class Bucket:Elements_On_The_Screen
    {
       public Stack<Point> dispersal;
        protected int color; // the color of the bucket
        public Bucket(Point start, int color)/// bucket without initalize (for moves)
        {
            this.start = start;
            this.color = color;
        ...
        public override void Draw( Bitmap bm, Graphics g) // in order to draw again the fill
            {
